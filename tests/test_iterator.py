...
from bktest.iterator import DateIterator


class DateIteratorTest(unittest.TestCase):

    def test_iter_empty(self):
        iterator = DateIterator(None, None, ExporterCollection(), False, [])
        with self.assertRaises(StopIteration):
            next(iter(iterator))

    def test_iter_double(self):
        iterator = DateIterator(datetime.date.today(), None, ExporterCollection(), False, [])
        iter(iterator)
    ...
    def test_should_skip_weekends_allow(self):
        iterator = DateIterator(None, None, ExporterCollection(), False, [], allow_weekends=True)
        self.assertFalse(iterator._should_skip_weekends(datetime.date(2024, 1, 20), False, []))

    def test_should_skip_weekends_week(self):
        iterator = DateIterator(None, None, ExporterCollection(), False, [], allow_weekends=False)
    ...
    def test_should_skip_weekends(self):
        ...
        self.assertEqual(postponned, [date])

    def test_should_skip_holidays_allow(self):
        iterator = DateIterator(None, None, ExporterCollection(), False, [], allow_holidays=True)
        self.assertFalse(iterator._should_skip_holidays(datetime.date(2023, 12, 25), False, []))

    def test_should_skip_holidays_week(self):
        iterator = DateIterator(None, None, ExporterCollection(), False, [], allow_holidays=False)
    ...
    def test_next(self):
        ...
        self.assertEqual(next(iterator), (datetime.date(2024, 1, 2), False, [datetime.date(2024, 1, 1)]))
        ...
        self.assertEqual(next(iterator), (datetime.date(2024, 1, 8), False, [datetime.date(2024, 1, 7)]))
